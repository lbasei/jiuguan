...
5. Adventure 页提供独立的伙伴信息墙入口，从 `/api/adventure-partners` 读取卡片；引导页的“进入联名游园”打开 `/collect/tavern-park`，展示酒鬼地图骨架：主题地标 + 可点摊位钉；点位数据来自 `adventure_partners`（含 zone / pin / booth_no），图片来自 `partner_media`。
